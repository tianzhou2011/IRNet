...
def get_experiment_space(args):
    space= {# Architecture parameters
            ...
            #'inner_size':hp.choice('n_hidden', [64,128,256,512]),
            ...
            'pooling_mode': hp.choice('pooling_mode', [ 'FNO2' ]),
...
def main(args):
    ...
    #Y_df = pd.read_csv(f'./data/{args.dataset}/S/df_y.csv')
    ...
    #---------------------------------------------- Directories ----------------------------------------------#
    output_dir = f'./results/multivariate/{args.dataset}_{args.horizon}/treeDRNet/'
    #output_dir = f'./results/univariate/{args.dataset}_{args.horizon}/treeDRNet/'
...
if __name__ == '__main__':
    ...
    # horizons_input =[96,96,96*2,96*2]
    # ILI_horizons = [24, 36, 48, 60]
    # ILI_horizons_input = [24, 24, 24, 24]
    # Exchange_horizons_input = [96,96,96,96]
    
    horizons = [720]
    horizons_input =[720]
    
    #datasets = ['ETTm2', 'ECL', 'Exchange', 'traffic', 'weather', 'ili']
    #datasets = ['ETTm2', 'ETTm1','ECL','Exchange', 'ETTh2', 'ETTh1']
    #datasets = ['traffic', 'weather', 'ili']
    datasets=['ETTm2']
    for dataset in datasets:
        # Horizon
        if dataset == 'ili':
            horizons_dataset = ILI_horizons
            horizons_input = ILI_horizons_input
        elif dataset == 'Exchange':
            horizons_input = Exchange_horizons_input
        else:
            horizons_dataset = horizons
        for horizon,horizon_input in zip(horizons_dataset,horizons_input):
            print(50*'-', dataset, 50*'-')
            print(50*'-', horizon, 50*'-')
            start = time.time()
            args.dataset = dataset
            args.horizon = horizon
            args.horizon_input=horizon_input
