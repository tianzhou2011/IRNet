...
class SpectralConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1,seq_len,kernel_size):
        ...
        self.index = list(range(0, int(seq_len//(2*kernel_size))))
        np.random.shuffle(self.index)
        self.index = self.index[:modes1]
        self.weights1 = nn.Parameter(self.scale * t.rand(in_channels, out_channels, len(self.index),dtype=t.cfloat))
        self.kernel_size = kernel_size
                          
        
    def compl_mul1d(self, input, weights):
        # (batch, in_channel, x ), (in_channel, out_channel, x) -> (batch, out_channel, x)
        return t.einsum("bi,io->bo", input, weights)

    def forward(self, x):
        batchsize = x.shape[0]
        #Compute Fourier coeffcients up to factor of e^(- something constant)
        x_ft = t.fft.rfft(x,dim=-1)

        # Multiply relevant Fourier modes
        out_ft = t.zeros(batchsize, self.out_channels, x.shape[-1]//(2*self.kernel_size) + 1, device=x.device,dtype=t.cfloat)
        for wi,i in enumerate(self.index):
            #print('x_ft shape',x_ft[:, :, i].shape)
            #print('self.weights1 shape',self.weights1[:,:,wi].shape)
            out_ft[:, :,wi] = self.compl_mul1d(x_ft[:, :, i], self.weights1[:,:,wi])
        #Return to physical space
        x = t.fft.irfft(out_ft, n=(x.shape[-1]//self.kernel_size))
        return x
...
class SpectralConv1d_interpolate(nn.Module):
    ...
    def forward(self, x):
        ...
        # Multiply relevant Fourier modes
        out_ft = t.zeros(batchsize, self.out_channels, x.shape[-1]//2 + 1, device=x.device,dtype=t.cfloat)
        for wi,i in enumerate(self.index):
            #print('x_ft shape',x_ft[:, :, i].shape)
            #print('self.weights1 shape',self.weights1[:,:,wi].shape)
            out_ft[:, :,wi] = self.compl_mul1d(x_ft[:, :, i], self.weights1[:,:,wi])
        #Return to physical space
        x = t.fft.irfft(out_ft, n=(x.shape[-1]))
        x = F.interpolate(x,scale_factor=1/self.kernel_size)
        return x

class SpectralConv1d_maxpool(nn.Module):
    def __init__(self, in_channels, out_channels, modes1,seq_len,kernel_size):
        super(SpectralConv1d_maxpool, self).__init__()
        ...
        self.modes1 = modes1  #Number of Fourier modes to multiply, at most floor(N/2) + 1
        self.scale = (1 / (in_channels*out_channels))
        self.index = list(range(0, int(seq_len//2)))
        np.random.shuffle(self.index)
        self.index = self.index[:modes1]
        self.weights1 = nn.Parameter(self.scale * t.rand(in_channels, out_channels, len(self.index),dtype=t.cfloat))
        self.kernel_size = kernel_size
...
class _NHITSBlock(nn.Module):
    ...
    def __init__(self, n_time_in: int, n_time_out: int, n_x: int,
                 ...
                 n_layers: int,  batch_normalization: bool, dropout_prob: float, activation: str,block_id:int):
        """
        """
        super().__init__()

        assert (pooling_mode in ['max','average','FNO','FNO_inter','FNO2'])
        ...
        if pooling_mode == 'max':
            ...
            self.pooling_layer = nn.AvgPool1d(kernel_size=self.n_pool_kernel_size,
                                                  stride=self.n_pool_kernel_size, ceil_mode=True)
        elif pooling_mode == 'FNO':
            self.pooling_layer = nn.Sequential()
            self.pooling_layer.add_module('spectral',SpectralConv1d_maxpool(in_channels=1, out_channels=1, modes1=240,seq_len=480,kernel_size=self.n_pool_kernel_size))
            self.pooling_layer.add_module('pool',nn.MaxPool1d(kernel_size=self.n_pool_kernel_size,
                                              stride=self.n_pool_kernel_size, ceil_mode=True))
        elif pooling_mode == 'FNO2':
            self.pooling_layer = SpectralConv1d(in_channels=1, out_channels=1, modes1=240,seq_len=480,kernel_size=self.n_pool_kernel_size)
        elif pooling_mode == 'FNO_inter':
            self.pooling_layer = SpectralConv1d_interpolate(in_channels=1, out_channels=1, modes1=240,seq_len=480,kernel_size=self.n_pool_kernel_size)
            #self.pooling_layer.add_module('maxpool',nn.MaxPool1d(kernel_size=self.n_pool_kernel_size,
                                             #stride=self.n_pool_kernel_size, ceil_mode=True))
        hidden_layers = []
    ...
    def forward(self, insample_y: t.Tensor, insample_x_t: t.Tensor,
        ...
        #print('processed0 insample_y shape',insample_y.shape)
        insample_y = insample_y.squeeze(1)
        #print('processed1 insample_y shape',insample_y.shape)
        #raise Exception('aaa')
        ...
        #print('processed2 insample_y shape',insample_y.shape)

        # Static exogenous
        if (self.n_s > 0) and (self.n_s_hidden > 0):
            x_s = self.static_encoder(x_s)
            insample_y = t.cat((insample_y, x_s), 1)
        #print('processed x_s shape',x_s.shape)
        #print('processed3 insample_y shape',insample_y.shape)
